...
object TraverseNotes extends App {
  ...
  val folded1 = Traverse[List[?]].foldLeft(List(1, 2, 3), List.empty[Int]) { (acc, num) =>
    num :: acc
  }
  println(s"Traverse[List[?]].foldLeft(List(1, 2, 3), Nil)((acc, num) => num :: acc) = ${folded1}")
  val folded2 = Traverse[List[?]].foldRight(List(1, 2, 3), cats.Eval.Zero) { (num, accEval) =>
    accEval.map(acc => num + acc)
  }
