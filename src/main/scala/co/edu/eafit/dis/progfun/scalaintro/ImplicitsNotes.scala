...
object ImplicitsNotes extends App {
  ...
  // Implicit values!
  // Implicit values are like regular values, except that they become part
  // of the implicit scope, as such you can ask for them implicitly.
  // Given this they may be any kind of value, including objects.
  // However, it is recommended that they be simple vals
  ...
  // parameter's type, the most specific one will be chosen
  // using the rules of static overloading resolution.
  ...
  // and why one should only implement the most concrete possible trait,
  // to ensure there is always only one implicit instance in scope.
  println()

  // Implicit parameters!
  // A function may have an implicit parameters list,
  // if, when calling the function, these parameters are not
  // explicitly specified, then the compiler will search for them
  // in the implicit scope.
  // There can only be one implicit parameters list, and it has to be
  // the last one - however, it may have multiple parameters,
  // but all them will be implicit.
  ...
  //       In real code, one must never use such a trivial type like Int
  //       for an implicit parameter. Given there can only be
  //       one implicit instance for the asked type.
  println()

  // Implicit conversions!
  // An implicit conversion is just a function of one argument which is marked
  // with the implicit keyword.
  // They will be used by the compiler to cats a value of the input type
  // to the return type, when the value is used in a place where is expected
  ...
  // Note: Given they work pretty much like black magic,
  //       its use is not recommended - and one must warn the compiler
  //       of its use by means of a special import that works as a feature flag.
  println()

  // Implicit classes!
  // Implicit classes are like regular classes whose constructor is
  ...
  // they are usually used together with value classes to provide them
  // whit out the need to allocate a new class.
  ...
  // provides a 'toIntOption' method, the compiler will search for
  // an implicit conversion from String to something wich provides
  // such method - in this case 'StringOps'.
  // And because 'StringOps' is a value class, the compiler optimize
  // the following line as a method call, passing the string as the argument.
